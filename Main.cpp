...
#pragma region Create Shaders and InputLayout:
	{
		...
		ifstream vsf("VertexShader.cso", ios::binary);
		ifstream psf("PixelShader.cso", ios::binary);

		vector<char> vsd = { std::istreambuf_iterator<char>(vsf), std::istreambuf_iterator<char>() };
		vector<char> psd = { std::istreambuf_iterator<char>(psf), std::istreambuf_iterator<char>() };

		device->CreateVertexShader(vsd.data(), vsd.size(), nullptr, &vertexShader);
		device->CreatePixelShader(psd.data(), psd.size(), nullptr, &pixelShader);
		...
		device->CreateInputLayout(ied, ARRAYSIZE(ied), vsd.data(), vsd.size(), &inputLayout);
